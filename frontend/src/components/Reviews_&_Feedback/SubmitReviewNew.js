...
export default function SubmitReview() {
  ...
  const validate = () => {
    ...
    if (wordCount < 5) e.push('Comment must be at least 5 words');
  ...
  const commentWordCount = comment.trim()
    ? comment
        .trim()
        .split(/\s+/)
        .filter(Boolean).length
    : 0;
  const needsMoreCommentWords = commentWordCount < 5;
      ...
      <div className="review-form-card">
        ...
        <form onSubmit={(e) => { e.preventDefault(); save(); }}>
          ...
          <div className="form-group">
            ...
            />
            <small>
              {comment.length}/3000 characters • {commentWordCount} word
              {commentWordCount === 1 ? '' : 's'}
            </small>
            {needsMoreCommentWords && (
              <p className="error">Please write at least 5 words to help other shoppers.</p>
            )}
          </div>
