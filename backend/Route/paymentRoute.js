...
// Admin utility: attempt to repair paid payments that are missing linked orders
router.post('/repair-missing-orders', requireAuth, (req, res, next) => {
  // only allow admins
  if (!req.user || !Array.isArray(req.user.roles) || !req.user.roles.includes('admin')) {
    return res.status(403).json({ message: 'Forbidden' });
  }
  next();
}, require('../Controlers/paymentController').repairMissingOrders);
